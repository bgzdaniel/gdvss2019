...
close_by = numpy.sum(radial[:, :] < 0.001, axis = 1)
...
closeBy = numpy.sum(radial_education[:, :] < 0.001, axis = 1)
...
subsetHealth = health
closeBy = numpy.sum(radial_health[:, :] < 0.001, axis = 1)
...
closeBy = numpy.sum(radial_leisure[:, :] < 0.001, axis = 1)
...
subsetPublic = public
closeBy = numpy.sum(radial_public[:, :] < 0.001, axis = 1)
...
closeBy = numpy.sum(radial_catering[:, :] < 0.001, axis = 1)
...
closeBy = numpy.sum(radial_shopping[:, :] < 0.001, axis = 1)
...
subsetMoney = money
closeBy = numpy.sum(radial_money[:, :] < 0.001, axis = 1)
...
def initial_map(new_housing):
	...
	return {
		"data":[
#			go.Scattermapbox(
#				lon = pointsofinterest["longitude"],
#				lat = pointsofinterest["latitude"],
#				text = pointsofinterest["text"],
#				mode = "markers",
...
app.layout = html.Div([
	...
	html.Div([
		dcc.Markdown('''**Multiplikatoren**'''),
	], style={"width": "15em", "padding": "1em"}),
	html.Div([
		dcc.Markdown('''Bildung'''),
		sliders[0],
	], style={"width": "15em", "padding": "1em", "display": "inline-block"}),
	html.Div([
		dcc.Markdown('''Gesundheit'''),
		sliders[1],
	], style={"width": "15em", "padding": "1em", "display": "inline-block"}),
	html.Div([
		dcc.Markdown('''Freizeit'''),
		sliders[2],
	], style={"width": "15em", "padding": "1em", "display": "inline-block"}),
	html.Div([
		dcc.Markdown('''Öffentliche Gebäude'''),
		sliders[3],
	], style={"width": "15em", "padding": "1em", "display": "inline-block"}),
	html.Div([
		dcc.Markdown('''Gastwirtschaft'''),
		sliders[4],
	], style={"width": "15em", "padding": "1em", "display": "inline-block"}),
	html.Div([
...
def update_map(hoverData, n_clicks, education_value, health_value, leisure_value, public_value, catering_value, shopping_value, money_value, clickData):
	...
	if (clickData != None and "customdata" in clickData["points"][0]):
		...
		ii, jj = numpy.where(radial[:, :] < 0.001)
		in_radius = pandas.DataFrame({"housing_id": ii, "poi_id": jj})
		print(in_radius.loc[in_radius["housing_id"] == housing_id, "poi_id"])
